...
\label{wc:lem:ternary-inner-weight-positive}
    For $k \leqslant n$ and $(a, n-a-c, c)$ a proper lower inner type, i.e. $0 \leqslant a, c \leqslant n$, $0<a+c\leqslant n$ and $a-k<c\leqslant a$ we have $\wn(a, c)>0$.
...
It is easy to show by induction on $n$ (with the base cases $k \leqslant n \leqslant n-k+1$ and inductive step of size $k$) that Theorem \ref{the_main_theorem} holds asymptotically, i.e. that for any $d, k$ constant and $n \rightarrow \infty$ we have (denoting by $A_{k, d}(n)$ the size of the largest subsets satisfying the conditions of Theorem \ref{the_main_theorem} for the given $k, d, n$):
\begin{equation*}
