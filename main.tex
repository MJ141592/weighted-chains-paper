...
\label{wc:lem:ternary-auxiliary-positive}
    For $n \geqslant k$ and $0 \leqslant c \leqslant a$, $0 \leqslant n-a-c$ and $0 < a+c$ we have $U_n(a, c)>0$.
...
$1:$ If $n=k$, since for an inner layer $a-n<c\leqslant n$ so we cannot have $a=n$ (as then $0<c$, but we also need $a+c \leqslant n$). Hence, we have $\un(c+n, a-n)=0$ and so $W_n(a, c)=U_n(a, c)>0$.
...
$2:$ For $n>k$, we have
    ...
    \begin{caseindent}
    $2.1:$ If both of the conditions are satisfied, then $(a-1, c),$ $(a, c)$ and $(a, c-1)$ are all lower inner types in $\{0, 1, 2\}^{n-1}$, and so by induction we have
        \begin{equation*}
    ...
    $2.2:$ If $c > a-1$, then (since $c \leqslant a$) we must have $a=c$. From the recursive definition of $U_n$ we have:
        \begin{equation*}
    ...
    $2.3:$ If $a-k \geqslant c-1$, then (since $a-k<c$) we must have $a=c+k-1$. In this case we have
        \begin{equation*}
