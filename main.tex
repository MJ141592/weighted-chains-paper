...
In working on and writing this paper, we used LLMs for spelling and typo checking, grammar proofreading, \LaTeX{} queries, searching literature, searching for binomial coefficient identities, generating \LaTeX{} code for the figures (corresponding to our detailed descriptions) and generating the Lean formalisation code and the corresponding blueprint (with GPT-5.6 Sol in ultra mode). We have checked manually that the Lean code is a faithful translation of the paper, and that the proofs are legitimate and follow the arguments of the paper. This formalisation process had the additional benefit of helping us to find several further minor typos in the statements and proofs. As a further experiment, we attempted a few efforts with both GPT-5.6 Sol and GPT-6 Astra in ultra mode at proving the cases beyond our main theorem, i.e. with $d \geqslant 3$, and the models made no noteworthy progress. No part of the mathematical content or the paper write-up was generated by a model.
...
\label{d1_lower_inner_positive}
    For $k \leqslant n$ and $n-k < 2a \leqslant n$ we have $U_n(a) - U_n(n-a-k) > 0.$
\end{lemma}
