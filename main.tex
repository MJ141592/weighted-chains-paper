...
\begin{caseindent}
    ...
    \end{multline*}
    which by induction is positive. \qedhere
\end{caseindent}
\end{proof}
...
We will show that for $(a, b, c)$ a proper lower type (i.e. with $n=a+b+c$, $0 \leqslant c \leqslant a,$ $0 < a+c \leqslant n$), $U_n(a, c)$ is positive. We do this in Lemma \ref{wc:lem:ternary-auxiliary-positive}. Before we prove this formally in the lemma, let us first discuss the proof strategy.

Let us call all lower types in the hypercube, except for $(0, n, 0)$, \emph{proper} lower types.
...
\end{itemize}
Hence, we will need to consider the cases when one of the above conditions doesn't hold. For $(a, b, c)$ a proper lower type, this can happen in the following cases: $c=0$, $a=0$, $b=0$, $a=c$. These are the special cases considered separately in the proof of Lemma \ref{wc:lem:ternary-auxiliary-positive}.

\begin{lemma}
\label{wc:lem:ternary-auxiliary-positive}
    For $n \geqslant k$, $0 \leqslant c \leqslant a$, $0 \leqslant n-a-c$ and $0 < a+c$ we have $U_n(a, c)>0$.
...
\begin{proof}
    ...
    \begin{caseindent}
        ...
        We have
            ...
            \end{multline*}
            Since $(n-1)-a-(a-1)=n-2a=n-a-c \geqslant 0$, $a > 0$, $a>a-1$ and finally $a+k+1>a-k$, we have $\un(a, a) \geqslant \uno(a, a-1)>0$ by induction. \qedhere
    \end{caseindent}
\end{caseindent}
\end{proof}
...
Now we need to show that for any proper lower inner type $(a, n-a-c, c)$ we have $U_n(a, c) - U_n(c+k, a-k) > 0.$ 

Let us recall Equation \ref{d2_u_n1}:
\begin{equation*}
    U_n(a, c)=\uno(a-1, c) + \uno(a, c)+\uno(a, c-1).
\end{equation*}
Similarly, we have:
...
\end{equation}
Hence, if $(a-1, n-a-c, c)$, $(a, n-a-c-1, c)$ and $(a, n-a-c, c-1)$ are all proper lower inner types, then clearly $\un(a, c)-\un(c+k, a-k)>0$. We will induct on $n$ (with $n=k$ the base case). For any $n>k$, we will need to carefully consider the cases when $(a, n-a-c, c)$ is a proper lower inner type (i.e. $0 \leqslant a, c \leqslant n$, $0<a+c\leqslant n$ and $a-k<c\leqslant a$) but one of $(a-1, n-a-c, c)$, $(a, n-a-c-1, c)$ and $(a, n-a-c, c-1)$ is not a proper lower inner type. This may happen if one of the following conditions is broken:
\begin{enumerate}
    \item $(a-1)-k < c \leqslant a-1$. Otherwise, $(a-1, n-a-c, c)$ is an outer type.
    \item $a-k < c-1 \leqslant a$. Otherwise, $(a, n-a-c, c-1)$ is an outer type.
    \item $1 < a+c$. Otherwise, $(a-1, n-a-c, c)$ or $(a, n-a-c, c-1)$ is the improper type $(0, n-1, 0)$.
    \item $a, c, n-a-c < n$. Otherwise, we get an empty type.
    \item $0 < a-1, n-1-a-c, c-1$. Otherwise, we get an empty type.
\end{enumerate}
We notice that for a proper lower inner type and $n>k$, we always have the following conditions satisfied: $a-1-k<c$, $c-1 \leqslant a$, $a, c, n-a-c < n$. We consider the remaining conditions being broken carefully in the full inductive proof below.
...
\begin{proof}
    ...
    $1:$ If $n=k$, since for an inner type $a-n<c\leqslant a$ we cannot have $a=n$ (as then $0<c$, but we also need $a+c \leqslant n$). Hence, we have $\un(c+n, a-n)=0$ and so $W_n(a, c)=U_n(a, c)>0$.

    $2:$ Let us now suppose that $n>k$.
        \begin{caseindent}
        $2.1:$ If all of the conditions $(1)-(4)$ are satisfied, then $(a-1, n-a-c, c),$ $(a, n-a-c-1, c)$ and $(a, n-a-c, c-1)$ are all lower inner types in $\{0, 1, 2\}^{n-1}$ (or empty, if one of $a$, $c$ or $n-a-c$ is $0$). Let us recall Equation \ref{d2_w_recursive} and the fact that at least one of $a, c, n-a-c$ is strictly positive. Hence, at least one of the three types $(a-1, n-a-c, c),$ $(a, n-1-a-c, c)$ and $(a, n-a-c, c-1)$ is a lower inner type in $\{0, 1, 2\}^{n-1}$, and so by induction its $\wno$ is positive, and so is $\wn(a, c)$.
        
        $2.2:$ If $c > a-1$ (i.e. condition $(1)$ is broken), then (since $c \leqslant a$) we must have $a=c$. From the recursive definition of $U_n$ we have:
            \begin{equation*}
        ...
        $2.3:$ If $a-k \geqslant c-1$ (i.e. condition $(2)$ is broken), then (since $a-k<c$) we must have $a=c+k-1$. In this case we have
            \begin{equation*}
            ...
            $2.4: 0 \geqslant a+c-1$, i.e. $a+c=1$. In this case (since $(a, n-a-c, c)$ is a lower type) we must have $a=1$ and $c=0$. However, then the type we consider is $(1, n-1, 0)$ and we have:
            \begin{equation*}
                \wn(1, 0) = \wno(0, 0) + \wno(1, 0) + \wno(1, -1).
            \end{equation*}
            We have $\wno(0,0)=0$ (as this is the improper type $(0, n-1,0)$) and $\wno(1, -1)=0$ (as this is an empty type). However, for $k \geqslant 2$ the type $(1, n-2, 0)$ is a proper lower inner type, and so $\wn(1, 0)=\wno(1, 0)>0$.
    
            We recall that condition $(4)$ is always satisfied, so we do not need to consider the (`empty') case when it is broken. \qedhere
            \end{caseindent}
\end{proof}
