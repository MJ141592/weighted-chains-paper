...
Hence, we will show that for any $n$, the unique largest antichains in $\mathbb{P}([n])$ are
\begin{equation*}
...
We note that sn interesting difference between this approach and the standard chain decomposition proof is that this one is symmetric, shifting the focus from constructing a decomposition to proving its properties.
...
\end{equation}
so $A_1$ is a $k$-separated set of maximal size, and similary for $A_2$.
