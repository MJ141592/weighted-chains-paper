...
Let $X_1, \ldots, X_n$ be disjoint sets. What is the size of the largest family of subsets $\mathcal{F} \subseteq \mathcal{P}(X_1 \cup \ldots \cup X_n)$ such that there are no $U, V \in \mathcal{F}$ s.t. $U \subsetneq V$ and $U \cap X_i = V \cap X_i$ for some $1 \leqslant i \leqslant n$?
...
Finally, in \cite{on_error_detection_in_asymmetric_channels}, Kovačević introduces a general coding-theory framework, where one is given an asymmetric channel over the alphabet $\{1, \dots, q\}$ and wishes to find (asymptotically) largest error-detecting code, where the errors can appear on at most $h$ coordinates, be of magnitude at most $a$ each and have the sum of magnitudes at most $t$. This phrasing is a generalisation of the problem discussed in this paper.
...
It may be helpful to visualise the types of $\{0, 1, 2\}^n$ on a \emph{staircase plot}, as shown on Figure \ref{figure_chain}. It shows the unit squares in the coordinate system (with the horizontal axis labelled as $a$ and the vertical axis labelled as $c$) whose coordinates satisfy $0 \leqslant a, c$ and $a+c \leqslant n$ (hence a staircase shape).
...
\begin{proof}
    ...
    Hence, the only types that require careful checking are $(a, n-a, 0)$ for $a\leqslant k$. If $a=0$, the type is $(0, n, 0)$ which contains only $\thespecialpoint$.
