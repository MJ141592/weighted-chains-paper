...
\label{weights_imply_theorem_lemma}
    For any $n, 1 < k \leqslant n,$ and $d$ if there exists an assignment of weights to good chains in $\thehypercube$ satisfying conditions (\ref{sum_1_condition}) and (\ref{non_negative_condition}), then any $k$-separated set in the hypercube is at most as large as $A_1$ (for $A_1$ defined as in Theorem \ref{the_main_theorem}).
...
Now we are ready to show that $W_n(a)>0$ for lower inner layers. We need to show that for $n-k < 2a \leqslant n$ we have $U_n(a) - U_n(n-a-k) > 0.$ Somewhat surprisingly, we will fix $k$ and show this by induction on $n$. Since we are interested in $k \leqslant n$, the base case of the induction is $n=k$.
...
\begin{proof}
    ...
    Hence, the only types that require careful checking are $(a, n-a, 0)$ for $a < k$. If $a=0$, the type is $(0, n, 0)$ which contains only $\thespecialpoint$.
...
\end{equation*}
and so for these types we have (for $c \leqslant a-k$):
\begin{multline}
...
We note that $U_n$ has the same base values and recursive formula as $W_n(a, c)$ for $c+k \leqslant a \leqslant n-c$ (i.e., lower outer types), so for these types we have $U_n(a, c)=W_n(a, c)$.
...
We note that if $n=a+b+c$, the three $\uno$s above correspond to types $(a-1, b, c)$, $(a, b-1, c)$ and $(a, b, c-1)$.

\subsection{Showing that chains starting at the outer types have positive weights}
\label{d2_outer_positive}

We will show that for $(a, b, c)$ a lower type different from $(0, n, 0)$ (i.e. $n=a+b+c$, $0 \leqslant c \leqslant a,$ $0 < a+c \leqslant n$), $U_n(a, c)$ is positive. We do this in Lemma \ref{wc:lem:ternary-auxiliary-positive}. Before we prove this formally in the lemma, let us first discuss the proof strategy.

Let us call all lower types in the hypercube, except for $(0, n, 0)$, \emph{proper} lower types.

From Equation \ref{d2_u_n1}, for any triple $(n, a, c)$ if we know that $\uno(a-1, c), \uno(a, c)$ and $\uno(a, c-1)$ are all positive, then clearly $\un(a, c)$ is also positive. We may hope that the three $\uno$ terms can be naturally shown positive by induction.

We will induct on $n$, treating $n=k$ as the base case. For most proper lower types $(a, b, c)$ in $\{0, 1, 2\}^n$ ($b=n-a-c$), the types $(a-1, b, c)$, $(a, b-1, c)$ and $(a, b, c-1)$ are proper lower types in $\{0, 1, 2\}^{n-1}$, and the inductive step is trivial. However, for some types $(a, b, c)$ one or more of the three types might no longer be lower, or not proper, or might be a trivial empty type. We will need to check for this carefully, but each case in itself will be straightforward to handle.

The three types $(a-1, b, c)$, $(a, b-1, c)$ and $(a, b, c-1)$ are all lower proper if:
\begin{itemize}
    ...
    \item $1 < a+c$. Otherwise, if $a+c=1$, then either $(a-1, b, c)$ or $(a, b, c-1)$ is the improper type $(0, n-1, 0)$.
    \item $c \leqslant a-1$. Otherwise, the type $(a-1, b, c)$ would be an upper type.
\end{itemize}
...
This lemma immediately implies that the weights of the chains starting at the outer lower types (and so, by symmetry, all outer types) are positive.
...
\begin{proof}
    ...
    $1:$ $n=k$. In this case, we have $U_n(a+k+1, c-k-1) = U_n(a+k+1, c-k)=0$ and so
        ...
        \begin{equation*}
            \frac{n!}{c!a!(n-a-c)!(n-c)(n-c+1)} ((a-c)(n-c) + a),
        \end{equation*}
        which is positive for $c \leqslant a \leqslant n-c$.
    
    $2:$ $n>k$. To do induction on $n$, let us recall \ref{d2_u_n1}.
    \begin{caseindent}
        $2.1:$ If we have: $0 \leqslant a-1, c-1, (n-1)-a-c$ and $c \leqslant a-1$, then $(a-1, n-a-c, c)$, $(a, n-a-c-1, c)$ and $(a, n-a-c, c-1)$ are all lower types in $\{0,1,2\}^{n-1}$ and so by induction each of the three summands is positive, and so is $U_n(a, c)$. Let us now consider case by case the `edge' cases when one of these conditions fail.
        ...
        $2.4:$ If $(n-1)-a-c < 0$, we must in fact have $a+c=n$. We may assume that $a, c > 0$. But then $\un(a, c) \geqslant \uno(a, c-1)>0$ by induction, which finishes the proof.
    
        $2.5:$ If $c > a-1$, we must in fact have $c=a$. Let us assume that $a>0$ (as $a=c=0$ has been discussed in the case $2.2$).
        We have
            \begin{multline*}
...
Now we need to show that for any proper (i.e. with $0 < a+c$) lower inner type $(a, n-a-c, c)$ we have $U_n(a, c) - U_n(c+k, a-k) > 0.$ 
...
\end{equation}
Hence, if $(a-1, n-a-c, c)$, $(a, n-1-a-c, c)$ and $(a, n-a-c, c-1)$ are all proper lower inner types, then clearly $\un(a, c)-\un(c+k, a-k)>0$. We will induct on $n$ (with $n=k$ the base case). For any $n>k$, we will need to carefully consider the cases when $(a, n-a-c, c)$ is a proper lower inner type (i.e. $0 \leqslant a, c \leqslant n$, $0<a+c\leqslant n$ and $a-k<c\leqslant a$) but one of $(a-1, n-a-c, c)$, $(a, n-1-a-c, c)$ and $(a, n-a-c, c-1)$ is not a proper lower inner type. This may happen if one of the following conditions is broken:
\begin{enumerate}
    ...
    \item $0 < a+c-1$. Otherwise, $(a-1, n-a-c, c)$ or $(a, n-a-c, c-1)$ is the improper type $(0, n-1, 0)$.
...
\begin{proof}
    We prove the lemma by induction on $n$, with $n=k$ being the base case.
...
\begin{proof}
    Let $L_i := \{X \subseteq [n]: \vert X\vert =i\}$ be called \emph{layer $i$}. For $0 \leqslant i \leqslant \frac{n}{2}$, the symmetric chains containing an element of $L_i$ are exactly those from $\mathcal{C}_0 \cup \ldots \cup \mathcal{C}_i$. Hence, the sum of weights of chains containing any element of $L_i$ is
    \begin{equation*}
