...
\end{equation*}
which can be shown by induction on $d$ with base case $d=1$ (when $L_i=\binom{n}{i}$ and so the unimodality follows from the unimodality of binomial coefficients).
...
\begin{proof}
    ...
    $1:$ If $n=k$, since for an inner layer $a-n<c\leqslant n$ we cannot have $a=n$ (as then $0<c$, but we also need $a+c \leqslant n$). Hence, we have $\un(c+n, a-n)=0$ and so $W_n(a, c)=U_n(a, c)>0$.
...
\begin{equation}
    A = \Big\{ x \in \{0,1, 2\} : \vert x\vert  \equiv n \ (\modd 2k+1) \Big\}.
\end{equation}
