...
Let us call a subset $A \subseteq \thehypercube$ `$k$-separated' if for any $x, y \in A$ with $x \leqslant y$, $x$ and $y$ differ on more than $k$ coordinates. What is the largest $k$-separated subset of the hypercube $\thehypercube$?
...
While working on the paper and writing it we only used Large Language Models for spellchecking, grammar proofreading, Latex querries, searching the literature and searching for binomial coefficient identities. We also used it to generate Latex code for the figures (that would correspond exactly to our description of a figure). No part of this note was written or rewritten by an AI model.
...
We briefly discuss another related problem, which was our initial motivation for coming up with the notion of $k$-separated sets.
...
We note that the size of layers is the larger, the closer they are to the middle of the hypercube: $1=\vert L_0\vert  \leqslant \vert L_1\vert  \leqslant \ldots \leqslant \vert L_{\fclass}\vert =\vert L_{\cclass}\vert  \geqslant \ldots \geqslant \vert L_{nd-1}\vert  \geqslant \vert L_{nd}\vert =1$.
...
\label{wc:def:type}
    For $x \in \thehypercube$, we say that $x$ is of type $(t_0 \ldots, t_n)$ if for any $0 \leqslant i \leqslant n$ exactly $t_i$ of $x\text{'s}$ coordinates equal $i$.

    We will also often treat a type tuple $(t_0, \ldots, t_n)$ (for $t_0+\ldots+t_n=d$) as the set of vertices of the hypercube of such type.

    According to this convention, a type tuple $(t_0, \ldots, t_n)$ with $t_i < 0$ for some $i$ is just an empty set.
\leanblueprint{type}
\end{definition}

Two vertices are of the same type if and only if their coordinates are a permutation of each other (i.e., types are orbits of the group of the coordinate permutation group).

There are $\binom{n}{t_0, \ldots, t_{n-1}}$ vertices of type $(t_0, \ldots, t_{n-1})$, where we define the multinomial symbol as:
\begin{equation*}
...
\begin{equation*}
    \binom{n}{a_1, \ldots, a_k} = \binom{n-1}{a_1, \ldots, a_k} + \binom{n-1}{a_1-1, \ldots, a_k} + \ldots + \binom{n-1}{a_1-1, \ldots, a_k-1}.
\end{equation*}
...
Similarly to calling layers lower/upper, outer/inner, we can call a type for example `lower' if it belongs to a lower type, etc. 
...
\label{the_weights_assigning_proposition}
    ...
    \begin{enumerate}
        ...
        \item \label{positive_A_condition} For any $x \in (A_1 \cup A_2) \setminus \thespecialpoint$ there exists a good chain of positive weight starting at through $x$.
    \end{enumerate}
    In the last condition, $y_i$ is closer to the middle of the cube than $x$ if and only if $\vert |y_i| - \frac{nd}{2} \vert < \vert |x| - \frac{nd}{2} \vert$.
...
We also note that these conditions make sense also when $2 \nmid n$, in which case simply $A_i \setminus \{\thespecialpoint\} = A_i$.
...
\begin{proof}
    ...
    We may draw this condition on a bipartite graph, whose vertices are points from the layers $L_{\fclass}$ and $L_{\cclass}$ and where two vertices $x \in L_{\fclass},$ $y \in L_{\cclass}$ are connected by an edge if and only $x \leqslant y$. Since this bipartite graph is connected, the only subsets of vertices of this graph which contain exactly one vertex from each edge are $L_{\fclass}$ and $L_{\cclass}$.
    
    Hence, we need either $L_{\fclass} \subseteq A'$ or $L_{\cclass} \subseteq A'$. Without loosing on generality, let us assume that the former is the case (and then $L_{\cclass} \cap A'=\emptyset$).
...
Then, we will verify that all weights $W_n(a)$ assigned this way are positive, i.e. that conditions (\ref{non_negative_condition})-(\ref{positive_A_condition}) of the Proposition hold (as discussed after the Proposition's statement).
...
\begin{itemize}
    ...
    \item Moreover, if $L_a$ is an inner layer, the chains starting at $L_{a+k}$ pass through $L_a$ but not through $L_{a-1}$.
\end{itemize}
...
Now we are ready to show that for lower inner layers $W_n(a)>0.$ We need to show that for $n-k < 2i \leqslant n$ we have $U_n(a) - U_n(a+k) > 0.$ Somewhat surprisingly, we will fix $k$ and show this by induction on $n$. Since we are interested in $k \leqslant n$, the base case of the induction is $n=k$.
...
\end{equation*}
then $a$ and $a-1$ are lower inner layers of $\{0,1,2\}^{n-1}$ and so
\begin{equation*}
...
\label{wc:def:basic-chain}
    Let $(x_1, \ldots, x_l)$ be a saturated chain of width $w$. We call this chain good if $l=dw+1$ and its subchains $(x_{id+1}, x_{id+2}, \ldots, x_{id+d+1})$ (for $0 \leqslant i \leqslant w-1$) are all of width $1$.
...
For a given $k$, we can also represent all basic chains starting at a lower outer type $(a, n-a-c, c)$ as shaded squares, as shown on the figure. For $k=2$, the following squares represent types that chains starting at type $(a, n-a-c, c)$ pass through: $(a, c)$, the square below it $(a-1, c)$, the square to its right $(a-1, c+1)$, the square below it $(a-2, c+1)$ and the square to its right $(a-2, c+2)$.
...
\label{positive_basic_enough_lemma}
    ...
    Moreover, for any $x \in A$, there exists a basic good chain starting at $x$.

    Finally, for any $x \notin A$, there exists $y \in A \setminus \{(1, \dots, 1)\}$, $y$ closer to the middle of the cube than $x$, and a basic good chain passing through $x$ and $y$.
...
Below we present a new proof of Sperner's theorem that we arrived at while working on this paper. It is the special case with $d=1$ and $k=n$. This section is self-contained and doesn't refer to the rest of the paper, so that it may be read independently.
...
Hence, we will show that for any $n$, the unique largest antichains in $[n]$ are
\begin{equation*}
    A_1 := \big\{X \subseteq [n]: \vert X\vert  = \frac{n-1}{2}\big\}
    \ \text{and} \
    A_2 := \big\{X \subseteq [n]: \vert X\vert  = \frac{n+1}{2}\big\}
\end{equation*}
(where if $2 \nmid n$, we have $A_1=A_2$).
...
Let us notice that for any $i$, $\{X \subseteq [n]: \vert X\vert =i\}$ is an orbit under the action of permuting $[n]$. Similarly, so is $\mathcal{C}_i$ for any $i$. It follows that for any $X, Y \subseteq [n]$ with $\vert X\vert =\vert Y\vert $ and any $i$ exactly the same number of symmetric chains from $\mathcal{C}_i$ contain $X$ and $Y$.
...
\begin{claim}
    The total weight of symmetric chains passing through any element of $[n]$ is $1$.
\end{claim}
...
Let $A \subseteq \mathcal{P}([n])$ be the largest antichain. We have
\begin{equation}
...
\end{equation}
where an equality can hold only if $A$ contains an element of each chain from $\mathcal{C}$. Similarly, since $A_1$ and $A_2$ satisfy this condition, we get
\begin{equation}
...
For $2 \mid n$, it immediately follows from Equation \ref{n_even_cn2_equation} that $\{X \subseteq [n]: \vert X\vert  = \frac{n}{2}\} \subseteq A$ and so $A=A_1=A_2$.

For $2 \nmid n$, it follows from Equation \ref{n_odd_cn2_equation} that for any $X \subseteq Y$, $\vert X\vert =\frac{n-1}{2}$, $\vert Y\vert =\frac{n+1}{2}$ $A$ must contain either $X$ or $Y$. Considering a bipartite graph of subsets of $[n]$ of size $\frac{n-1}{2}$ and $\frac{n+1}{2}$, we may easily see that we either have $\{X \subseteq [n]: \vert X\vert  = \frac{n-1}{2}\} \subseteq A$ (and so $A=A_1$) or $\{X \subseteq [n]: \vert X\vert  = \frac{n+1}{2}\} \subseteq A$ (and so $A=A_2$).

This finishes the proof.

We note that interesting differences between this approach and the standard chain decomposition proof is that this one is symmetric, and also that the focus is shifted from constructing a decomposition to proving its properties.
...
\begin{equation*}
    X = C_1 \cup \ldots \cup C_p, \ Y = D_1 \cup \ldots \cup D_q.
\end{equation*}
...
\end{equation}
so $A_1$ one of the largest $k$-separated sets.
