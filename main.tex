...
Gunby, He, Narayanan and Spiro in \cite{more_coding_theory} studied from the perspective of coding theory problems related to Sperner's theorem and reminiscent of its generalisation explored here. They looked for the largest antichain $A$ in $\{0, 1\}^n$ satisfying a further condition than any $x, y \in A$ have Hamming distance at least $k$ (i.e., differ on at least $k$ indices) and show that $\vert A\vert=O_k(2^nn^{-k/2})$ for odd $k$.
...
\begin{proof}
    ...
    $2:$ Let us now suppose that $n>k$.
        ...
        $2.3:$ If $a-k \geqslant c-1$ (i.e. condition $(2)$ is broken), then (since $a-k<c$) we must have $a=c+k-1$. In this case we have
            ...
            We recall than condition $(4)$ is always satisfied, so we do not need to consider the (`empty') case when it is broken.
            \end{caseindent}
\end{proof}
...
Another interesting line of investigation would be to try different methods for constructing bounded-width (unweighted) chain decompositions of the hypercube, or to show that it does not exist.
